...
const auth = (p: Partial<Authority> = {}): Authority => ({
  ...
  network: { mode: "any" },
  subprocess: { allowed: true, commands: ["npm"] },
  credentials: [{ handle: "secret:a", scopes: ["read"] }],
  sideEffects: ["issue.comment"],
  delegation: { allowed: true, maxDepth: 3 },
  ...p,
});

describe("authority", () => {
  it("attenuates read/write and runtime policy", () => {
    const child = intersectAuthority(
      auth(),
      auth({ filesystem: { read: ["/repo"], write: [] } }),
      auth({ capabilities: ["test"] }),
      auth(),
    );
    expect(child.filesystem.write).toEqual([]);
    expect(child.filesystem.read).toEqual(["/repo"]);
    expect(child.capabilities).toEqual(["test"]);
  });
  ...
  it("intersects credential scopes and drops empty scope intersections", () => {
    expect(
      intersectAuthority(
        ...
        auth({
          credentials: [{ handle: "secret:a", scopes: ["read", "write"] }],
        }),
    ...
    expect(
      intersectAuthority(
        auth({ credentials: [{ handle: "secret:a", scopes: ["read"] }] }),
        auth({ credentials: [{ handle: "secret:a", scopes: ["write"] }] }),
        auth({ credentials: [{ handle: "secret:a" }] }),
        auth({ credentials: [{ handle: "secret:a" }] }),
      ).credentials,
    ).toEqual([]);
  ...
  it("cannot create network or credentials", () => {
    expect(
      isAuthoritySubset(
        auth({ network: { mode: "bounded", destinations: ["api.example"] } }),
        auth({ network: { mode: "none" } }),
      ),
    ).toBe(false);
    expect(
      isAuthoritySubset(
        auth({ credentials: [{ handle: "secret:b" }] }),
        auth(),
      ),
    ).toBe(false);
  });
  ...
  it("can prohibit further delegation", () => {
    const child = intersectAuthority(
      auth(),
      auth({ delegation: { allowed: false, maxDepth: 0 } }),
      auth(),
      auth(),
    );
    expect(child.delegation).toEqual({ allowed: false, maxDepth: 0 });
    expect(isAuthoritySubset(child, auth())).toBe(true);
  });
});
